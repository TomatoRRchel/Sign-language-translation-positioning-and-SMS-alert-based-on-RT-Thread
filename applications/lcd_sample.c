       ...
       // rt_kprintf("page %d\n",page);
        ...
        if(page==1){
            ...
            if(reflag){
                ...
                show_page=Page;
                follow=1;
          /*      temp=lcd_page[Page].head.next;
             //   rt_kprintf("reshow index:%d,loc:%d\n",temp->index,temp->loc);
                while(temp->len!=0){
                    if(temp->index!=255) //8+32
                ...
                /*len_sum_temp=0;
                for(char j=0;j<show_index;j++)
                {  len_sum_temp+=len[j];//24
                LCD_ShowIntNum(40-16-12, 44+(len_sum_temp-len[j])*40, j+1, 1, BLACK, BROWN, 32);
                Display_GB2312_String(40-12, 44+(len_sum_temp-len[j])*40, 32,show[j] , BLACK, BROWN);
                }*/
            }
            if(!follow){
            ...
            if(*recv_box!='@'){
                ...
                if(lcd_page[Page].free >= lcd_object[show_index]->len){/// 如果要翻页 则进入这些程序 不翻页 直接插入链表
                    ...
                    show_index++;
                    show[show_index]=rt_malloc(str_len(&show[show_index-1][(lcd_page[Page-1].free)*12]));
